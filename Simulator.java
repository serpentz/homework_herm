...
class Simulator {
	...
	public void setupInitialTrainPositions(Scanner trainScanner) {
		...
		while (trainScanner.hasNextLine()) {
			...
			// if it isn't blank, break it into parts (the train name followed by the car
			// names)
			String[] parts = s.split(" ");
			// create a new train and add it to the list of all trains (the list is called
			// "trains")
			Train train = new Train(parts[0]);
			trains.append(parts[0], train);
